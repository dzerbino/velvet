...
By default, there are only two short read categories, but this variable can be extended to your needs. Towards the top of the \emph{src/globals.h} file, you will find a line:

\begin{verbatim}
CATEGORIES		2
\end{verbatim}

Simply change the values to the desired one and re-compile as explained in \ref{sec:compilation}.
...
By default, hash-lengths are limited to 31bp, but you can push up this limit by adjusting the MAXKMERLENGTH parameter on top of the \emph{src/globals.h} file and re-compiling as explained in \ref{sec:compilation}.
